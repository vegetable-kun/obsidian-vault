...
> [!info] 为什么是 14 篇而不是 200 多篇
> 本篇原先散布着 200+ 条指向知识碎片的 wikilink（`HyDE`、`Multi-query`、`查询优化`、`Checkpoint`…）。它们其实是**同一主题下的子节**，拆成独立笔记只会让读者在 8 个文件之间来回跳。
> 因此正文中这些碎片已改为**普通文本**（知识点名称原样保留，勾选框仍然可用），==只在主题级保留簇级链接==；下面的表是它们的完整归属 —— 写笔记时按簇写一篇厚的即可。
