...
> **阶段目标**：能独立调 API、理解 LLM 基础原理、跑通第一个带工具调用的 Agent
> 
...
**实践**：用 Pydantic 定义一个 Agent 的消息类型系统（SystemMessage、UserMessage、ToolMessage、AssistantMessage）

#### 1.1.2 异步编程

| 知识点 | 具体内容 | 学习方法 | 资源 |
|---|---|---|---|
| asyncio 基础 | async/await、event loop、Task | 写一个并发爬虫 | Real Python |
...
**实践**：用 asyncio + aiohttp 写一个能并发调用多个 LLM API 的调度器
...
**实践**：用策略模式实现一个支持 OpenAI/Anthropic/本地模型 切换的 LLM 调用类
...
| 知识点 | 具体内容 | 学习方法 | 资源 |
|---|---|---|---|
| Self-Attention | Q/K/V 计算、缩放点动手画注意力矩阵 | 《动手学深度学习》 | |
...
**实践**：写一个脚本，同一问题发给 5 个模型，对比回答质量、延迟、价格
...
**实践**：为每种技术写 5 个 Prompt 模板，建立个人 Prompt 库
...
**实践**：用 tiktoken 写一个工具，输入文本返回 token 数和预估费用
...
**实践**：封装一个支持 OpenAI/Anthropic/Ollama 的统一调用类

---

## 第二阶段：RAG 系统（第 4-6 周）

> **阶段目标**：完整掌握 RAG 链路，能搭建知识库问答系统
> 
...
**实践**：用 OpenAI Embedding API 把 100 篇文档向量化，存入 Chroma
...
文档加载 → 分块 → 向量化 → 存入向量库
                              ↓
用户提问 → 向量化 → 检索 Top-K → 拼接上下文 → LLM 生成 → 返回答案
```

**实践**：搭一个能回答 Hermes Agent 文档的问答系统
...
**实践**：用 RAGAS 评估自己的 RAG 系统，输出评估报告

---

## 第三阶段：Agent 框架（第 7-9 周）

> **阶段目标**：掌握 LangGraph + CrewAI，能构建多 Agent 系统
> 
...
| 组件 | 说明 | 学习方法 |
|---|---|---|
| Model I/O | Model、Prompt、Parser | 写一个完整的调用链 |
...
**实践**：用 LangGraph 实现电商问数 Agent（NL2SQL）
...
**实践**：用 CrewAI 构建"研究员 + 审稿人 + 撰稿人"三 Agent 系统

---

## 第四阶段：项目实战（第 10-12 周）

> **阶段目标**：产出 2-3 个可展示的项目，丰富简历
> 
...
**简历描述**："基于 LangGraph 的智能客服系统，支持多轮对话、知识库检索、自动转人工，日均处理 100+ 咨询"
...
**简历描述**："多 Agent 协作研究系统，支持自动搜索、总结、生成报告，接入 WebSocket 实时进度回传"
...
**简历描述**："基于 Hermes Agent 的个人助手，集成 MCP 协议与长期记忆，支持多平台接入"

---

## 第五阶段：大模型微调（第 13-14 周）

> **阶段目标**：掌握 LoRA 微调流程，能针对特定场景优化模型
> 
> **检验标准**：完成一个 LoRA 微调案例，对比微调前后效果
